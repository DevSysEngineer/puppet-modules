...
- For value-resolution conditionals, put the larger generic, inherited, default, or fallback block first and put the smaller direct explicit assignment in the final `else`. When optional empty strings are treated like unset values, prefer explicit absent-or-empty conditions such as `if ($value == undef or ($value != undef and $value == '')) { ...fallback... } else { $value_correct = $value }`.

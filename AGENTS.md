...
These rules are the single source of truth for inline comments in Puppet manifests, ERB templates, generated configs, shell scripts, and monitoring checks. Language-specific sections may define resource order, setup order, and public API documentation, but must not restate the general comment policy.
...
- Add short comments directly above non-obvious Puppet resource groups, `exec` resources, derived values, conditional directive lists, delegated resources, cleanup paths, shell escaping boundary preparation, shell parsing, classification, status aggregation, fallback handling, output-building blocks, and meaningful helper functions in monitoring checks.
- In monitoring checks, put a short comment directly above each non-trivial function. The comment should name the function's operational purpose and, when relevant, its inputs, output format, Nagios impact, or preserved external contract.
- For new or materially changed Puppet manifests, shell scripts, shell templates, and generated configs, match the comment density of the surrounding well-documented code. Do not leave newly introduced helper code, wrapper code, function blocks, assignment blocks, or resource groups as long uncommented sequences when a reviewer would need extra context to follow the flow.

...
- `ProtectControlGroups=strict`: unsafe for services that manage, create, delegate, or write Linux control groups, including container managers, VM or container runtimes, nested service managers, workload supervisors, and agents that intentionally move processes between cgroups. Use this only on supported systemd releases that accept the `strict` value. Treat monitoring, resource-accounting, orchestration, and troubleshooting tools that inspect host cgroup layout as higher risk until their read-only and namespace behavior is tested.
